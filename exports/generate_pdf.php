...
$id = $_POST['payment_id'] ?? $_GET['id'] ?? null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['items'])) {
    ...
    // Veritabanından varsayılan veriyi kullan
    $items[] = [
        'name' => $data['project_title'] . ' - Profesyonel Hizmet Bedeli',
...
<body class="bg-slate-100 p-4 md:p-12">
    ...
    <div
        class="max-w-4xl mx-auto bg-white shadow-2xl rounded-sm overflow-hidden border-t-[16px] border-indigo-600 container">
        <div class="p-12">
            <!-- Header Section -->
            <div class="flex justify-between items-start mb-16">
                <div>
                    ...
                    <div class="text-sm text-slate-500 space-y-1 max-w-xs">
                        <p class="font-medium text-slate-700">
                            <?php echo nl2br(htmlspecialchars($settings['address'] ?? '')); ?></p>
                ...
                <div class="text-right">
                    ...
                    <div class="space-y-1">
                        <p class="text-sm font-bold text-zinc-900">Teklif No: <span
                                class="font-normal text-slate-500">#OFF-<?php echo str_pad($data['id'], 5, '0', STR_PAD_LEFT); ?></span>
                        </p>
                        <p class="text-sm font-bold text-zinc-900">Tarih: <span
                                class="font-normal text-slate-500"><?php echo date('d.m.Y'); ?></span></p>
                        <p class="text-sm font-bold text-zinc-900">Geçerlilik: <span
                                class="font-normal text-slate-500"><?php echo date('d.m.Y', strtotime('+15 days')); ?></span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-12 mb-16">
                <div class="bg-slate-50 p-8 rounded-2xl border border-slate-100">
                    <h3 class="text-[10px] font-bold text-indigo-600 uppercase tracking-widest mb-4">Müşteri Bilgileri
                    </h3>
                    <div class="space-y-2">
                        <p class="text-xl font-bold text-zinc-900">
                            <?php echo htmlspecialchars($data['client_name'] ?? ''); ?></p>
                        <p class="text-sm font-medium text-slate-600">
                            <?php echo htmlspecialchars($data['client_company'] ?? ''); ?></p>
                        <p class="text-sm text-slate-500">
                            <?php echo nl2br(htmlspecialchars($data['client_address'] ?? 'Adres belirtilmemiş.')); ?>
                        </p>
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <p class="text-2xl font-bold text-zinc-900 mb-3">
                        <?php echo htmlspecialchars($data['project_title'] ?? ''); ?></p>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b-2 border-zinc-900">
                            <th class="py-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                Hizmet Açıklaması</th>
                            <th
                                class="py-4 text-right text-[10px] font-bold text-slate-400 uppercase tracking-widest w-32">
                                Birim Fiyat</th>
                            <th
                                class="py-4 text-right text-[10px] font-bold text-slate-400 uppercase tracking-widest w-24">
                                Adet</th>
                            <th
                                class="py-4 text-right text-[10px] font-bold text-slate-400 uppercase tracking-widest w-32">
                                Toplam</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php foreach ($items as $item): ?>
                            <tr>
                                ...
                                <td class="py-8 text-right text-slate-600 font-medium">
                                    ₺<?php echo number_format($item['price'], 2); ?></td>
                                <td class="py-8 text-right text-slate-600 font-medium"><?php echo $item['qty']; ?></td>
                                <td class="py-8 text-right font-bold text-zinc-900 text-lg">
            ...
            <div class="flex justify-end mb-24">
                <div class="w-64 space-y-3">
                    <div class="flex justify-between items-center text-slate-500 text-sm">
                        ...
                        <span class="font-semibold text-zinc-900">₺<?php echo number_format($total_amount, 2); ?></span>
                    </div>
                    <div class="flex justify-between items-center text-slate-500 text-sm">
                        <span>KDV (%0)</span>
                    ...
                    <div class="pt-4 border-t-2 border-zinc-900 flex justify-between items-center">
                        ...
                        <span
                            class="text-2xl font-black text-indigo-600">₺<?php echo number_format($total_amount, 2); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-12 pt-12 border-t border-slate-100">
                ...
                <div class="text-right flex flex-col justify-end">
                    <p class="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Bu belge dijital olarak
                        oluşturulmuştur</p>
                    <p class="text-xs font-bold text-zinc-900">
                        <?php echo htmlspecialchars($settings['company_name'] ?? 'EkinCRM'); ?> Sistemi</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto mt-8 text-center no-print">
        <p class="text-slate-400 text-xs uppercase tracking-widest">© <?php echo date('Y'); ?>
            <?php echo htmlspecialchars($settings['company_name'] ?? 'EkinCRM'); ?> - Tüm Hakları Saklıdır.</p>
    </div>
</body>
