    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <header class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Ödemeler</h1>
                                    ...
                                    <td class="px-6 py-4 text-right space-x-2">
                                        <div class="relative inline-block text-left dropdown-container">
                                            <button onclick="toggleDropdown(this)"
                                                class="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
                                                <i data-lucide="more-horizontal" class="w-5 h-5"></i>
                                            </button>
                                            <div
                                                class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 dropdown-menu transition-colors">
                                                <div class="py-1">
                                                    <a href="?update_status=Ödeme Alındı&id=<?php echo $payment['id']; ?>"
                                                        class="block px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">Ödeme
                                                        Alındı Olarak İşaretle</a>
                                                    <a href="?update_status=Bekliyor&id=<?php echo $payment['id']; ?>"
                                                        class="block px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">Bekliyor
                                                        Olarak İşaretle</a>
                                                    <a href="?update_status=Fatura Kesildi&id=<?php echo $payment['id']; ?>"
                                                        class="block px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">Fatura
                                                        Kesildi Olarak İşaretle</a>
                                                    <a href="generate_pdf.php?id=<?php echo $payment['id']; ?>" target="_blank"
                                                        class="block px-4 py-2 text-sm text-indigo-600 dark:text-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-800 font-medium">Teklif
                                                        Oluştur (PDF)</a>
                                                    <a href="generate_invoice.php?id=<?php echo $payment['id']; ?>"
                                                        target="_blank"
                                                        class="block px-4 py-2 text-sm text-indigo-600 dark:text-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-800 font-medium">Fatura
                                                        Oluştur</a>
                                                </div>
                                            </div>
                                        </div>
    ...
    <div id="addPaymentModal"
        ...
        <div
            ...
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">Yeni Ödeme Kaydı</h3>
